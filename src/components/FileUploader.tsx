...
export const FileUploader = ({
  ...
  return (
    <div className="space-y-4">
      <div
        {...getRootProps()}
        className={`rounded-md bg-slate-100 px-[28px] mx-[233px] py-[10px] my-[2px] cursor-pointer hover:bg-slate-200 transition-colors ${
      ...
      {files.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {files.map((file, index) => (
            <div key={index} className="pdf-preview p-4 relative bg-slate-50 rounded-md">
              <button
